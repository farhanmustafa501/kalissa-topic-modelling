...
from app.services.ai import generate_topic_name, generate_topic_insights
...
def _chunk_and_embed_documents(
	session, 
	collection_id: int, 
	job: Optional[DiscoveryJob]
) -> List[Chunk]:
	"""
	...
	for doc_idx, doc in enumerate(docs, start=1):
		...
		logger.debug(
			"Document chunked",
			extra={"doc_id": doc.id, "num_chunks": len(chunk_texts)}
		)
...
def _get_representative_chunks(
	...
	logger.debug(
		"Representative chunks selected",
		extra={"cluster_size": len(chunks), "top_n": len(representative_texts)}
	)
...
def _build_relationships(topic_id_to_centroid: Dict[int, np.ndarray]) -> List[Tuple[int, int, float]]:
	...
	logger.info(
		"Topic relationships built",
		extra={"num_topics": len(ids), "num_relationships": len(edges)}
	)
...
def run_discovery(session: SessionLocal, collection_id: int, job: DiscoveryJob) -> None:
	...
	try:
		...
		for chunk, label in zip(chunks_with_embeddings, labels):
		...
		for label, cluster_chunks in cluster_to_chunks.items():
			...
			max_docs = max(len(set(c.document_id for c in chunks)) for chunks in cluster_to_chunks.values()) if cluster_to_chunks else 1
		...
		logger.exception("Topic discovery failed", extra={"collection_id": collection_id, "job_id": job.id, "error": str(e)})
		# Persist failure
