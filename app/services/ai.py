...
def generate_topic_name(representative_chunks: list[str]) -> dict[str, Any]:
    ...
    chunk_texts = "\n\n".join(f"<CHUNK_{i+1}>\n{chunk[:1500]}" for i, chunk in enumerate(representative_chunks[:5]))
...
{{
  ...
  "related_concepts": ["concept1", "concept2", "concept3"]
    ...
    try:
        logger.debug(
            "Generating topic insights with GPT-4o-mini",
            extra={"topic_name": topic_name, "num_chunks": len(representative_chunks)},
        )
        resp = client.chat.completions.create(
...
{context_text}"""
    ...
    try:
        logger.debug(
            "Generating Q&A answer with GPT-4o", extra={"question_length": len(question), "num_chunks": len(chunks)}
        )
        resp = client.chat.completions.create(
