...
class TestCollectionsAPI:
    ...
    def test_create_collection(self, client, db_session):
        ...
        data = {"name": "New Test Collection", "description": "Test description"}
        response = client.post("/api/collections", data=json.dumps(data), content_type="application/json")
    ...
    def test_create_collection_form_data(self, client, db_session):
        ...
        response = client.post("/api/collections", data=data, content_type="application/x-www-form-urlencoded")
        assert response.status_code == 201
        result = json.loads(response.data)
...
class TestDocumentsAPI:
    ...
    def test_add_documents_json(self, client, sample_collection, db_session):
        ...
        data = {"documents": [{"title": "Doc 1", "content": "Content 1"}, {"title": "Doc 2", "content": "Content 2"}]}
        response = client.post(
            f"/api/collections/{sample_collection.id}/documents", data=json.dumps(data), content_type="application/json"
        )
        assert response.status_code == 200
        result = json.loads(response.data)
    ...
    def test_upload_files(self, client, sample_collection, db_session):
        ...
        data = {"files": [(io.BytesIO(file_content), "test.txt")]}
        response = client.post(
    ...
    def test_add_documents_form_json(self, client, sample_collection, db_session):
        ...
        data = {
            "documents_json": json.dumps(
                [{"title": "Doc 1", "content": "Content 1"}, {"title": "Doc 2", "content": "Content 2"}]
            )
        }
        response = client.post(
    ...
    def test_add_documents_rejected_missing_title(self, client, sample_collection, db_session):
        ...
        data = {"documents": [{"title": "Valid Doc", "content": "Content"}, {"content": "No title"}]}  # Missing title
        response = client.post(
            f"/api/collections/{sample_collection.id}/documents", data=json.dumps(data), content_type="application/json"
        )
        assert response.status_code == 200
    ...
    def test_add_documents_title_truncation(self, client, sample_collection, db_session):
        ...
        data = {"documents": [{"title": long_title, "content": "Content"}]}
        response = client.post(
            f"/api/collections/{collection_id}/documents", data=json.dumps(data), content_type="application/json"
        )
        assert response.status_code == 200
    ...
    def test_add_documents_content_truncation(self, client, sample_collection, db_session):
        ...
        data = {"documents": [{"title": "Test", "content": long_content}]}
        response = client.post(
            f"/api/collections/{collection_id}/documents", data=json.dumps(data), content_type="application/json"
        )
        assert response.status_code == 200
...
class TestUIRoutes:
    ...
    def test_ui_topic_with_chunks(self, client, sample_topic, sample_document, db_session):
        ...
        chunk = Chunk(document_id=sample_document.id, chunk_index=0, text="Test chunk")
    ...
    def test_ui_topic_with_related_topics(self, client, sample_topic, db_session):
        ...
        related_topic = Topic(collection_id=sample_topic.collection_id, name="Related Topic")
...
class TestDiscoveryAPI:
    ...
    def test_delete_discovery_job(self, client, sample_collection, sample_discovery_job, db_session):
        ...
        response = client.delete(f"/api/collections/{sample_collection.id}/discover/last_job")
        assert response.status_code == 200
        data = json.loads(response.data)
...
class TestTopicsAPI:
    ...
    def test_topic_qa(self, mock_qa, client, sample_topic, sample_document, db_session):
        ...
        response = client.post(
            f"/api/topics/{sample_topic.id}/qa", data=json.dumps(data), content_type="application/json"
        )
        assert response.status_code == 200
    ...
    def test_topic_qa_exception(self, mock_qa, client, sample_topic, sample_document, db_session):
        ...
        db_session.add(doc_topic)
        chunk = Chunk(document_id=sample_document.id, chunk_index=0, text="Test chunk", embedding=[0.1] * 1536)
        db_session.add(chunk)
        db_session.commit()
